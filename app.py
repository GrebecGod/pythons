from cgitb import text
from statistics import multimode
from kivy.app import App
from kivy.uix.gridlayout import GridLayout
from kivy.uix.label import Label
from kivy.uix.image import Image
from kivy.uix.button import Button
from kivy.uix.textinput import TextInput
import webbrowser
import time

class NONE(App):
    def build(self):
        self.window = GridLayout()
        self.window.cols=1
        self.window.size_hint=(0.6, 0.7)
        self.window.pos_hint={"center_x":0.5, "center_y":0.5}
        #add widgets to window
        self.window.add_widget(Image(source=("NONE.png")))
        self.greeting=Label(text="Hi,i am None!")
        self.window.add_widget(self.greeting)
        self.user=TextInput(multiline=False)
        self.window.add_widget(self.user)
        self.button=Button(text="GREET ")
        self.button.bind(on_press=self.callback)
        self.window.add_widget(self.button)
        self.button=Button(text="Search")
        self.button.bind(on_press=self.search)
        self.window.add_widget(self.button)
        return self.window
    def callback(self, instance):
        if self.user.text == "Nigga":
            self.greeting.text = "Hello monkey!"
        if self.user.text== "stop":
            exit()
        else :
            self.greeting.text = "Hello " + self.user.text + "!"
    def search(self, instace):
            self.greeting.text= "Searching for "+ self.user.text
            url='https://google.com/search?q=' + self.user.text 
            webbrowser.get().open(url)
if __name__ == "__main__":
    NONE().run()